...
server <- function(input, output, session) {
    ...
    # Cache phylostratum data to avoid repeated loading
    phylomap_data <- NULL
    phylo_legend_data <- NULL
    ...
    observeEvent(input$load_pathway, {
        ...
        tryCatch({
            ...
            pathway_data <- parse_kegg_pathway_with_hsa(pathway_id, use_cached = TRUE, comprehensive_mapping = comprehensive_mapping)
    ...
    # Handle EMERALD alignment requests from individual gene dropdowns
    observeEvent(input$emerald_request, {
        if (!is.null(input$emerald_request) && !is.null(comprehensive_mapping)) {
            req_data <- input$emerald_request
            first_gene <- req_data$firstGene
            first_uniprot <- req_data$firstUniProt
            second_gene <- req_data$secondGene
            container_id <- req_data$containerId
            
            # Get UniProt ID for second gene
            second_gene_mapping <- comprehensive_mapping[
                !is.na(comprehensive_mapping$hgnc_symbol) & 
                comprehensive_mapping$hgnc_symbol == second_gene & 
                !is.na(comprehensive_mapping$uniprotswissprot) & 
                comprehensive_mapping$uniprotswissprot != "", 
            ]
            
            if (nrow(second_gene_mapping) > 0) {
                second_uniprot <- second_gene_mapping$uniprotswissprot[1]
                
                # Create proper EMERALD alignment URL
                emerald_url <- paste0(
                    "https://algbio.github.io/emerald-ui/?seqA=", first_uniprot, 
                    "&seqB=", second_uniprot, "&alpha=0.75&delta=8"
                )
                
                # Send JavaScript to update the specific container
                session$sendCustomMessage("updateEmeraldContainer", list(
                    containerId = paste0(container_id, "_container"),
                    html = paste0(
                        "<a href='", emerald_url, "' target='_blank' style='color: #007bff; text-decoration: underline;'>",
                        "🔗 Compare ", first_gene, " vs ", second_gene, "</a><br>",
                        "<small style='color: #666;'>", first_uniprot, " vs ", second_uniprot, "</small>"
                    )
                ))
                
                showNotification(paste("EMERALD alignment ready:", first_gene, "vs", second_gene), type = "message")
            } else {
                # Handle case where second gene doesn't have UniProt ID
                session$sendCustomMessage("updateEmeraldContainer", list(
                    containerId = paste0(container_id, "_container"),
                    html = "<small style='color: #999;'>No UniProt ID found for second gene</small>"
                ))
            }
        }
    })
        ...
        tryCatch({
            ...
            if (!is.null(values$nodes) && nrow(values$nodes) > 0) {
                if ("hgnc_symbol" %in% colnames(values$nodes)) {
                    ...
                    for (i in 1:nrow(values$nodes)) {
                        node <- values$nodes[i, ]
                        
                        # Check if this is a consolidated node with all_gene_symbols
                        if (!is.na(node$all_gene_symbols) && node$all_gene_symbols != "") {
                            # Extract individual gene symbols from consolidated node
                            individual_genes <- strsplit(node$all_gene_symbols, ",")[[1]]
                            individual_genes <- trimws(individual_genes)
                            individual_genes <- individual_genes[!is.na(individual_genes) & individual_genes != ""]
                            pathway_genes <- c(pathway_genes, individual_genes)
                        } else if (!is.na(node$hgnc_symbol) && node$hgnc_symbol != "") {
                            # Regular node - use hgnc_symbol as is
                            pathway_genes <- c(pathway_genes, node$hgnc_symbol)
                        }
                    }
            ...
            if (!is.null(selected_node_id) && length(selected_node_id) > 0 && selected_node_id != "") {
                ...
                if (nrow(selected_node) > 0) {
                    ...
                    if (!is.na(node_type) && node_type == "gene") {
                        # GENE NODE: Handle both single genes and consolidated gene nodes
                        
                        # Check if this is a consolidated node (has all_gene_symbols field)
                        is_consolidated <- !is.na(selected_node$all_gene_symbols[1]) && 
                                         selected_node$all_gene_symbols[1] != ""
                        ...
                        if (is_consolidated) {
                            ...
                            # For consolidated nodes, get interactions of the consolidated node itself
                            # not of individual genes (since individual genes don't exist as separate nodes)
                            
                            # Use the consolidated node's primary hgnc_symbol for interaction lookup
                            consolidated_symbol <- selected_node$hgnc_symbol[1]
                            
                            interaction_data <- get_interacting_genes(
                                selected_gene = consolidated_symbol,
                                pathway_edges = values$edges,
                                pathway_nodes = values$nodes,
                                gene_id_type = "symbol"
                            )
                            
                            # Get interacting genes and their colors
                            all_interacting_genes <- interaction_data$interacting_genes
                        ...
                        if (length(all_genes_to_plot) > 0) {
                            # Handle colors based on node type
                            if (is_consolidated) {
                                # For consolidated nodes, use proper color scheme
                                ...
                                # Create plot title
                                plot_title <- paste0("Expression Profiles: ", selected_gene, " + ", 
